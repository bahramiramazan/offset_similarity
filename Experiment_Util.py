...
def plot_semantic_space_questions(model,model_name,abstract_spaces_dic,sim_f='cosine_similarity',n_size=50,plot=False,easy=False):
    ...
    if plot==False:
        ...
        # Create sample data with longer names
        Category=[]
        values=[]
        ...
        #plt.show() # Used to display the plot in a typical environment
...
def check_r_head_tail_concepts(all_related):
    ...
    print('category_counts',category_counts.head())
    print('count_',count_)
    ...
    # We map 'Frequency' to the x-axis and 'Category' to the y-axis
    sns.barplot(x='Frequency', y='Category', data=category_counts, palette='mako',errorbar=None)

    # # 4. Add labels to the bars for clarity
    # for index, row in category_counts.iterrows():
    #     plt.text(row['Frequency'] + 0.2, index, str(row['Frequency']), 
    #              color='black', ha='left', va='center', fontsize=11)

    #plt.title('Frequency of Items by Category', fontsize=16)
...
def make_conceptqa_questions(test_or_dev='test'):
    benchmarks='wikidata'
    train_eval='train'
    ...
    Questions =get_conceptqa_questions(abstract_spaces_dic,n_size=2)
    all_data=[]
    ...
    for qi,question in enumerate(Questions):
        #dict_keys(['q', 'answer', 'choices', 'key']
        q=question['q']
...
def evaluate_chatgpt():
    ...
    for da in ['conceptqa',]:
        dataname=da
        all_questions,solution_set =evaluation_mtcq(da) if da  in lexical_data  else  return_json_dic(dn=dataname) if da not in ['conceptqa',] else make_conceptqa_questions()

        file='essential_files/answers'+dataname+'_gpt-5-nano.json'
        file='essential_files/answers'+dataname+'_40-mini.json'
...
def plot_agreement_heat_map(data,modelnames):
    ...
    # --- 1. Recreate the Data ---
    # The models are the same for rows and columns (a correlation/similarity matrix)
    models=modelnames
    # models = [
    ...
    # The data is read row by row from the image
    # data = [
    #     [1, 0.63, 0.68, 0.59, 0.68, 0.65, 0.59, 0.6, 0.62, 0.64],
    #     [0.63, 1, 0.72, 0.65, 0.68, 0.68, 0.64, 0.85, 0.72, 0.74],
    #     [0.68, 0.72, 1, 0.69, 0.73, 0.69, 0.69, 0.7, 0.68, 0.73],
    #     [0.59, 0.65, 0.69, 1, 0.63, 0.65, 0.69, 0.64, 0.6, 0.6],
    #     [0.68, 0.68, 0.73, 0.63, 1, 0.75, 0.63, 0.67, 0.7, 0.73],
    #     [0.65, 0.68, 0.69, 0.65, 0.75, 1, 0.63, 0.67, 0.67, 0.67],
    #     [0.59, 0.64, 0.69, 0.69, 0.63, 0.63, 1, 0.63, 0.61, 0.6],
    #     [0.6, 0.85, 0.7, 0.64, 0.67, 0.67, 0.63, 1, 0.71, 0.72],
    #     [0.62, 0.72, 0.68, 0.6, 0.7, 0.67, 0.61, 0.71, 1, 0.74],
    #     [0.64, 0.74, 0.73, 0.6, 0.73, 0.67, 0.6, 0.72, 0.74, 1]
    # ]
    ...
    # Create the heatmap
    # The model in the image uses 'viridis' or a similar perceptually uniform colormap, 
    # but a divergent one like 'crest' or 'vlag' is also common for correlation. 
    # I'll use a variation of 'crest' which looks similar to the image's color scheme.
    sns.heatmap(
        df,
        annot=True,              # Show the values on the heatmap
        fmt=".2f",               # Format the numbers to 2 decimal places
        cmap="crest",            # Colormap that resembles the one in the sample
        linewidths=0.5,          # Add lines between cells for clarity
        cbar=True,               # Show the color bar
        vmin=0.60,               # Set min value for color scale (as seen in image)
        vmax=1.00                # Set max value for color scale
    )

    # Set the title
    plt.title('Offset Similarity Agreement between Models for SAT', fontsize=16)

    # Rotate x-axis labels for better visibility
    plt.xticks(rotation=70)
    # Ensure y-axis labels are horizontally aligned
    plt.yticks(rotation=0)

    # Optional: Adjust layout to prevent labels from being cut off
    plt.tight_layout()
...
def interagreement_between_models(m1,m2,dn,plot=True):
    ...
    if plot:
        ...
        plt.savefig('images/sim_dis/'+str(m1)+'and'+str(m2)+str(dn)+'.png')
        plt.close()
...
def solve_analogies(modelname,dn,not_corrct_terms,model=None,model_type='baseline',sim_f='offset_cosine_similarity',bayesian_analysis=False):
    ...
    for word in data_test_EVALution:
        ...
        #choice_d=[w_fast[t['w1']]-w_fast[ t['w2']]  for t in choice]
        cos = nn.CosineSimilarity(dim=-1, eps=1e-6)
        similarity=[]
        pca_arr=[stem_d,]
    ...
    # 1. Create your data (Contingency Table)
    # Rows: Phone, Computer
    ...
    # Let's assume your CSV has two columns: 'City' and 'Preference'
    # df = pd.read_csv('your_data.csv')

    # For this example, let's create a "many rows" dataset
    data = {
    ...
    # This turns thousands of rows into a simple summary table
...
def plot_sim_with_abs(abstract,model_name,all_abstracts,all_words):
    ...
    for d in SRE_Analogy:
        ...
        if model_name in all_models_names:
            ...
            # stem_d_p=model.get_word_vector(stem_w1) -model.get_word_vector(stem_w4)
            # stem_d_p_=model.get_word_vector(stem_w3) -model.get_word_vector(stem_w2)
...
def same_rel_sim(model_name,all_related):
    ...
    import numpy as np
    import math
    # head_tail_sim_all=rel_dic[k]['head_tail_sim']
    # offsetsim_all=rel_dic[k]['offsetsim']
