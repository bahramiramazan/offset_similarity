...
def split_word_analogy_data2():
        ...
        file='essential_files/localdatasets/analogy_sampled.json'
...
def process_file(args,filename, data_type,Relation_counter,data_selected,tokenizer=None):
    ...
    if data_selected=='conll':
        ...
        if data_selected=='BLESS':
            BLESS = load_from_disk('essential_files/localdatasets/BLESS')
            datasets=BLESS
            temp='train' if data_type=='train' else 'test'
            data=datasets[temp]
        elif data_selected=='CogALexV':
            CogALexV = load_from_disk('essential_files/localdatasets/CogALexV')
            datasets=CogALexV
            temp='train' if data_type=='train' else 'test'
            data=datasets[temp]
        elif data_selected=='EVALution':
            EVALution = load_from_disk('essential_files/localdatasets/EVALution')
            datasets=EVALution
            temp='train' if data_type=='train' else 'test'
            data=datasets[temp]

        elif data_selected=='ROOT09':
            ROOT09 = load_from_disk('essential_files/localdatasets/ROOT09')
            datasets=ROOT09
            temp='train' if data_type=='train' else 'test'
            data=datasets[temp]

        elif data_selected=='KandH_plus_N':
            KandH_plus_N = load_from_disk('essential_files/localdatasets/KandH_plus_N')
            ...
            file='essential_files/localdatasets/semeval_2012.json'
            file='essential_files/localdatasets/semeval_2012.json'
